...
public class OpenSearchTaskHandler {
    ...
    public void put(final Collection<SinkRecord> records) {
        ...
        for (final var record : records) {
            ...
            try {
                bulkOperationBuilder.buildFor(record).ifPresent(op -> bulkIngester.add(op, record));
    ...
    private void createIndex(final String indexName, final SinkRecord record) {
        try {
            ...
            throw new ConnectException("Couldn't create index " + indexName, e);
        }
    }
    ...
    private void createDataStreamIndex(final String indexName, final SinkRecord record) {
        config.dataStreamExistingIndexTemplateName().ifPresentOrElse(userProvidedTemplate -> {
            try {
                ...
                throw new ConnectException("Couldn't check that template " + userProvidedTemplate + " exists", e);
            }
        ...
        try {
            ...
            throw new ConnectException("Couldn't create data stream", e);
        }
    }
